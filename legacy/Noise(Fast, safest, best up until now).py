...
if __name__ == '__main__':
    while True:
        word = input("Word: ")
        #word = "Wie nennt man eine französische Militärparade? Eine Massenevakuierung!"
        am = int(input("Runs: "))
        seed = "mySeed"
        ...
        for i in range(am):
            ...
            print(f"\rDurchgang: {i + 1}, Time: {round(time.time() - start_time, 4)} ({round(float(time.time() - start_time)/float(i+1),4)} per pass)", end="")
